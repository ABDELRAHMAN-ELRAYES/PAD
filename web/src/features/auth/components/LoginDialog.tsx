...
import type { AuthenticatePayload } from "@/features/auth/types/api";
import { useAuth } from "@/features/auth/hooks/use-auth";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { authenticateSchema } from "@/features/auth/types/schema";
    ...
    form.handleSubmit(async (data) => {
      try {
        ...
        if (error instanceof ApiError) {
          form.setError("password", { message: error.message || "Incorrect email or password" });
        } else {
          form.setError("password", { message: "An unexpected error occurred. Please try again." });
        }
      }
    })();
  };
      ...
      <DialogContent className="sm:max-w-[460px] p-0 gap-0 rounded-3xl border border-border shadow-2xl bg-background text-foreground">
        <div className="px-6 pt-7 pb-6">
          <DialogHeader className="items-center text-center">
            ...
            <DialogTitle className="text-xl">Sign in to PAD</DialogTitle>
            ...
            {signInStep === "email" ? (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && canContinueSignIn) {
                      e.preventDefault();
                      handleContinue();
                    }
                  }}
