    ...
    return (
        ...
        >
            ...
            <div className={cn(
                ...
                {!isCollapsed && (
                    <h1 className="text-4xl text-black font-extrabold tracking-tight truncate pl-1">PAD</h1>
                )}
                <button
                    onClick={onToggle}
            ...
            <div className="p-3 shrink-0">
                <Tooltip>
                    <TooltipTrigger asChild>
                        <button
                            ...
                            className={cn(
                                ...
                                !activeIdeaId && "bg-accent border-solid border-accent-foreground/20",
                                isCollapsed ? "w-9 h-9 justify-center mx-auto" : "w-full gap-2 px-3 py-2 text-sm font-medium"
                            )}
                        >
                    ...
                    {isCollapsed && <TooltipContent side="right">New Idea</TooltipContent>}
                </Tooltip>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2 no-scrollbar">
                ...
                <div 
                    className={cn(
                        ...
                        isCollapsed && activeIdeaId ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                    )}
                >
                    <div className="overflow-hidden flex flex-col space-y-2">
                        <div className="w-8 h-px bg-border my-2 mx-auto shrink-0" />
                        {activeIdeaId && SECTION_ITEMS.map((item) => {
                            const isActive = activeSection === item.id;
                            const isDisabled = ideaStatus !== "confirmed" && item.id !== "overview";
                                ...
                                <Tooltip key={'collapsed-' + item.id}>
                                    <TooltipTrigger asChild>
                                        <button
                                            ...
                                            disabled={isDisabled}
                                            className={cn(
                                                "flex items-center rounded-lg transition-colors shrink-0 mx-auto justify-center w-9 h-9",
                                                isActive
                                    ...
                                    <TooltipContent side="right" sideOffset={8}>
                                        {isDisabled ? "Confirm idea first" : item.label}
                                    </TooltipContent>
                                </Tooltip>
                            );
                        })}
                    ...
                    <div className="overflow-hidden space-y-0.5">
                        ...
                        {isLoading ? (
                            ...
                            <div className="text-center py-6 shrink-0">
                                ...
                                <p className="text-xs text-muted-foreground whitespace-nowrap">No ideas yet</p>
                            </div>
                        ) : (
                            ideas.map((idea) => {
            ...
            <div 
                className={cn(
                    ...
                    !isCollapsed && activeIdeaId ? "grid-rows-[1fr] opacity-100 pb-2 pt-2 border-t-border" : "grid-rows-[0fr] opacity-0 pb-0 pt-0 border-t-transparent pointer-events-none"
                )}
                style={{ display: "grid" }}
            >
                <div className="overflow-hidden space-y-0.5">
